...
const SideBar = () => {
    ...
    const sideBarParentRef = useRef<React.MutableRefObject<HTMLDivElement | null>>(null);
    const sideBarChildRef = useRef<React.MutableRefObject<HTMLDivElement | null>>(null);
    const sideBarCloseButtonRef = useRef<React.MutableRefObject<HTMLButtonElement | null>>(null);
    ...
    useEffect(() => {
        const closeOnAwayClick = (event) => {
            if (
                ...
                if (
                    sideBarParentRef.current.contains(event.target) &&
                    !sideBarChildRef.current.contains(event.target) &&
                    !sideBarCloseButtonRef.current.contains(event.target) &&
